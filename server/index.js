...
const run= async() => {
  try {
    ...
    const authenticMenu = client.db('authenticFood').collection('products');
    const reviews = client.db('authenticFood').collection('reviews');

    app.get('/menu', async(req,res) => {
        const menu = await authenticMenu.find().toArray()
